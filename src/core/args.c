...
int cu_filters_match(const cu_filter *filters, const char *s)
{
    ...
    for (const cu_filter *f = filters; f; f = f->next) {
        int eq = f->strict ? 0 == strcmp(s, f->text)
                           : NULL != strstr(s, f->text);
        if (f->invert ? !eq : eq)
            return 1;
    }
    return 0;
...
static void add_filter(cu_filter **list, const char *text, int strict, int invert)
{
...
static int add_group_dot_name(cu_args *a, int ac, const char *const *av, int *i,
                              const char *parameter_name, int strict, int invert)
{
...
static void add_test_from_verbose_output(cu_args *a, int ac, const char *const *av,
                                         int *i, const char *prefix)
{
    ...
    if (comma) {
        ...
        name = name_len > 2 ? strndup(name_start + 2, name_len - 2) : strdup("");
    } else {
        group = strdup(field);
        name = strdup("");
    }
...
int cu_args_parse(cu_args *a, int argc, const char *const *argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (0 == strcmp(arg, "-v")) a->verbose = 1;
        else if (0 == strcmp(arg, "-vv")) a->very_verbose = 1;
        else if (0 == strcmp(arg, "-c")) a->color = 1;
        else if (0 == strcmp(arg, "-p")) a->run_separate_process = 1;
        else if (0 == strcmp(arg, "-b")) a->reversing = 1;
        else if (0 == strcmp(arg, "-lg")) a->list_groups = 1;
        else if (0 == strcmp(arg, "-ln")) a->list_names = 1;
        else if (0 == strcmp(arg, "-ll")) a->list_locations = 1;
        else if (0 == strcmp(arg, "-ri")) a->run_ignored = 1;
        else if (0 == strcmp(arg, "-f")) a->crash_on_fail = 1;
        else if (0 == strcmp(arg, "-e") || 0 == strcmp(arg, "-ci")) a->rethrow_exceptions = 0;
        else if (starts_with(arg, "-r")) set_repeat_count(a, argc, argv, &i);
        else if (starts_with(arg, "-g")) add_filter(&a->group_filters, parameter_field(argc, argv, &i, "-g"), 0, 0);
        else if (starts_with(arg, "-t")) correct = add_group_dot_name(a, argc, argv, &i, "-t", 0, 0);
        else if (starts_with(arg, "-st")) correct = add_group_dot_name(a, argc, argv, &i, "-st", 1, 0);
        else if (starts_with(arg, "-xt")) correct = add_group_dot_name(a, argc, argv, &i, "-xt", 0, 1);
        else if (starts_with(arg, "-xst")) correct = add_group_dot_name(a, argc, argv, &i, "-xst", 1, 1);
        else if (starts_with(arg, "-sg")) add_filter(&a->group_filters, parameter_field(argc, argv, &i, "-sg"), 1, 0);
        else if (starts_with(arg, "-xg")) add_filter(&a->group_filters, parameter_field(argc, argv, &i, "-xg"), 0, 1);
        else if (starts_with(arg, "-xsg")) add_filter(&a->group_filters, parameter_field(argc, argv, &i, "-xsg"), 1, 1);
        else if (starts_with(arg, "-n")) add_filter(&a->name_filters, parameter_field(argc, argv, &i, "-n"), 0, 0);
        else if (starts_with(arg, "-sn")) add_filter(&a->name_filters, parameter_field(argc, argv, &i, "-sn"), 1, 0);
        else if (starts_with(arg, "-xn")) add_filter(&a->name_filters, parameter_field(argc, argv, &i, "-xn"), 0, 1);
        else if (starts_with(arg, "-xsn")) add_filter(&a->name_filters, parameter_field(argc, argv, &i, "-xsn"), 1, 1);
        else if (starts_with(arg, "-s")) correct = set_shuffle(a, argc, argv, &i);
