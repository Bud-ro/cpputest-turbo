...
/* cpputest-turbo lite: SimpleString shim, no std::string interop. Method
 * bodies live in the library (src/shim/simplestring.cpp, the single C++
 * TU, built -fno-exceptions -fno-rtti so the archive needs no libstdc++)
 * — user translation units parse only declarations. */
