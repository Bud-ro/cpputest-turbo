...
/* cpputest-turbo lite: header-only SimpleString (no std::string interop,
 * no C++ translation units in the library — libCppUTest.a is pure C).
 * Rendering rules delegate to the C core for byte parity with the
 * assertion engine. */
...
inline bool ss_isSpace(char ch)
{
    return ch == ' ' || ch == '\t' || ch == '\n' || ch == '\r' || ch == '\f' ||
           ch == '\v';
}

inline bool ss_isDigit(char ch)
{
    return ch >= '0' && ch <= '9';
}
...
inline SimpleString takeOwnership(char *s)
{
