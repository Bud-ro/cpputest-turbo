...
{
    static const cu_test_ops ops = {CppUTestShimCreate, CppUTestShimDestroy,
                                    CppUTestShimSetup, CppUTestShimBody,
...
class UtestShell
{
  public:
    ...
    {
        return (node_.is_ignored && !node_.run_ignored) ? "IGNORE_TEST"
                                                        : "TEST";
    }
    ...
    {
        return node_.next ? static_cast<UtestShell *>(node_.next->user)
                          : NULLPTR;
    }
    ...
    void assertTrue(bool condition, const char *checkString,
                    const char *conditionString, const char *text,
                    const char *fileName, size_t lineNumber)
    {
        cu_assert_true(condition, checkString, conditionString, text, fileName,
                       lineNumber);
    }
    ...
    void assertCstrEqual(const char *expected, const char *actual,
                         const char *text, const char *fileName,
                         size_t lineNumber)
    {
        cu_assert_cstr_equal(expected, actual, text, fileName, lineNumber);
    }
    void assertCstrNEqual(const char *expected, const char *actual,
                          size_t length, const char *text, const char *fileName,
                          size_t lineNumber)
    {
        cu_assert_cstr_nequal(expected, actual, length, text, fileName,
                              lineNumber);
    }
    void assertCstrNoCaseEqual(const char *expected, const char *actual,
                               const char *text, const char *fileName,
                               size_t lineNumber)
    {
        cu_assert_cstr_nocase_equal(expected, actual, text, fileName,
                                    lineNumber);
    }
    void assertCstrContains(const char *expected, const char *actual,
                            const char *text, const char *fileName,
                            size_t lineNumber)
    {
        cu_assert_cstr_contains(expected, actual, text, fileName, lineNumber);
    }
    void assertCstrNoCaseContains(const char *expected, const char *actual,
                                  const char *text, const char *fileName,
                                  size_t lineNumber)
    {
        cu_assert_cstr_nocase_contains(expected, actual, text, fileName,
                                       lineNumber);
    }
    ...
    void assertLongLongsEqual(cpputest_longlong expected,
                              cpputest_longlong actual, const char *text,
                              const char *fileName, size_t lineNumber)
    {
        cu_assert_longlongs_equal(expected, actual, text, fileName, lineNumber);
    }
    void assertUnsignedLongLongsEqual(cpputest_ulonglong expected,
    ...
    void assertSignedBytesEqual(signed char expected, signed char actual,
                                const char *text, const char *fileName,
                                size_t lineNumber)
    {
        cu_assert_signed_bytes_equal(expected, actual, text, fileName,
                                     lineNumber);
    }
    void assertPointersEqual(const void *expected, const void *actual,
                             const char *text, const char *fileName,
                             size_t lineNumber)
    {
        cu_assert_pointers_equal(expected, actual, text, fileName, lineNumber);
    }
    void assertFunctionPointersEqual(void (*expected)(), void (*actual)(),
                                     const char *text, const char *fileName,
                                     size_t lineNumber)
    {
        cu_assert_functionpointers_equal((void (*)(void))expected,
                                         (void (*)(void))actual, text, fileName,
                                         lineNumber);
    }
    ...
    void assertBinaryEqual(const void *expected, const void *actual,
                           size_t length, const char *text,
                           const char *fileName, size_t lineNumber)
    {
        cu_assert_binary_equal(expected, actual, length, text, fileName,
                               lineNumber);
    }
    void assertBitsEqual(unsigned long expected, unsigned long actual,
                         unsigned long mask, size_t byteCount, const char *text,
                         const char *fileName, size_t lineNumber)
    {
        cu_assert_bits_equal(expected, actual, mask, byteCount, text, fileName,
                             lineNumber);
    }
    ...
    void assertCompare(bool comparison, const char *checkString,
                       const char *comparisonString, const char *text,
                       const char *fileName, size_t lineNumber)
    {
        cu_assert_compare(comparison, checkString, comparisonString, text,
                          fileName, lineNumber);
    }
...
extern "C" inline void *CppUTestShimCreate(cu_test *t)
{
    return static_cast<void *>(
        static_cast<UtestShell *>(t->user)->createTest());
}

extern "C" inline void CppUTestShimDestroy(cu_test *t, void *fixture)
{
    static_cast<UtestShell *>(t->user)->destroyTest(
        static_cast<Utest *>(fixture));
}

class TestInstaller
{
  public:
    TestInstaller(UtestShell &shell, const char *groupName,
