...
#define CHECK_TRUE_LOCATION(condition, checkString, conditionString, text, file, line)\
  do { cu_fast_count_check(); \
       if (condition) {} else cu_fail_check(checkString, conditionString, text, file, line); } while(0)

#define CHECK_FALSE_LOCATION(condition, checkString, conditionString, text, file, line)\
  do { cu_fast_count_check(); \
       if (condition) cu_fail_check(checkString, conditionString, text, file, line); } while(0)
...
#define CHECK_EQUAL_LOCATION(expected, actual, text, file, line)\
  do { if ((expected) != (actual)) { \
      ...
      cu_fast_count_check(); \
      cu_fail_equals_strings(StringFrom(expected).asCharString(), StringFrom(actual).asCharString(), text, file, line); \
  } \
  else \
  { \
    cu_fast_count_check(); \
...
#define STRCMP_EQUAL_LOCATION(expected, actual, text, file, line)\
  do { cu_fast_count_check(); \
       cu_strcmp_check((expected), (actual), text, file, line); } while(0)
...
#define STRNCMP_EQUAL_LOCATION(expected, actual, length, text, file, line)\
  do { cu_fast_count_check(); \
       cu_strncmp_check((expected), (actual), (length), text, file, line); } while(0)
...
#define LONGS_EQUAL_LOCATION(expected, actual, text, file, line)\
  do { long cu_fp_e = (long)(expected); long cu_fp_a = (long)(actual); \
       cu_fast_count_check(); \
       if (cu_fp_e == cu_fp_a) {} else cu_fail_longs(cu_fp_e, cu_fp_a, text, file, line); } while(0)

#define UNSIGNED_LONGS_EQUAL_LOCATION(expected, actual, text, file, line)\
  do { unsigned long cu_fp_e = (unsigned long)(expected); unsigned long cu_fp_a = (unsigned long)(actual); \
       cu_fast_count_check(); \
       if (cu_fp_e == cu_fp_a) {} else cu_fail_unsigned_longs(cu_fp_e, cu_fp_a, text, file, line); } while(0)
...
#define LONGLONGS_EQUAL_LOCATION(expected, actual, text, file, line)\
  do { cpputest_longlong cu_fp_e = (cpputest_longlong)(expected); cpputest_longlong cu_fp_a = (cpputest_longlong)(actual); \
       cu_fast_count_check(); \
       if (cu_fp_e == cu_fp_a) {} else cu_fail_longlongs(cu_fp_e, cu_fp_a, text, file, line); } while(0)

#define UNSIGNED_LONGLONGS_EQUAL_LOCATION(expected, actual, text, file, line)\
  do { cpputest_ulonglong cu_fp_e = (cpputest_ulonglong)(expected); cpputest_ulonglong cu_fp_a = (cpputest_ulonglong)(actual); \
       cu_fast_count_check(); \
       if (cu_fp_e == cu_fp_a) {} else cu_fail_unsigned_longlongs(cu_fp_e, cu_fp_a, text, file, line); } while(0)
...
#define SIGNED_BYTES_EQUAL_LOCATION(expected, actual, text, file, line)\
  do { signed char cu_fp_e = (signed char)(expected); signed char cu_fp_a = (signed char)(actual); \
       cu_fast_count_check(); \
       if (cu_fp_e == cu_fp_a) {} else cu_fail_signed_bytes(cu_fp_e, cu_fp_a, text, file, line); } while(0)
...
#define POINTERS_EQUAL_LOCATION(expected, actual, text, file, line)\
  do { const void *cu_fp_e = (const void *)(expected); const void *cu_fp_a = (const void *)(actual); \
       cu_fast_count_check(); \
       if (cu_fp_e == cu_fp_a) {} else cu_fail_pointers(cu_fp_e, cu_fp_a, text, file, line); } while(0)
...
#define FUNCTIONPOINTERS_EQUAL_LOCATION(expected, actual, text, file, line)\
  do { void (*cu_fp_e)() = (void (*)())(expected); void (*cu_fp_a)() = (void (*)())(actual); \
       cu_fast_count_check(); \
       if (cu_fp_e == cu_fp_a) {} else cu_fail_functionpointers((void (*)(void))cu_fp_e, (void (*)(void))cu_fp_a, text, file, line); } while(0)
...
#define DOUBLES_EQUAL_LOCATION(expected, actual, threshold, text, file, line)\
  do { double cu_fp_e = (double)(expected); double cu_fp_a = (double)(actual); double cu_fp_t = (double)(threshold); \
       cu_fast_count_check(); \
       if (cu_doubles_eq(cu_fp_e, cu_fp_a, cu_fp_t)) {} else cu_fail_doubles(cu_fp_e, cu_fp_a, cu_fp_t, text, file, line); } while(0)
...
#define MEMCMP_EQUAL_LOCATION(expected, actual, size, text, file, line)\
  do { cu_fast_count_check(); \
       cu_memcmp_check((expected), (actual), (size), text, file, line); } while(0)
...
#define BITS_LOCATION(expected, actual, mask, text, file, line)\
  do { unsigned long cu_fp_m = (unsigned long)(mask); size_t cu_fp_n = sizeof(actual); \
       unsigned long cu_fp_e = (unsigned long)(expected); unsigned long cu_fp_a = (unsigned long)(actual); \
       cu_fast_count_check(); \
       if ((cu_fp_e & cu_fp_m) == (cu_fp_a & cu_fp_m)) {} else cu_fail_bits(cu_fp_e, cu_fp_a, cu_fp_m, cu_fp_n, text, file, line); } while(0)
