...
#define SIGNED_BYTES_EQUAL_LOCATION(expected, actual, text, file, line)\
  do { UtestShell::getCurrent()->assertSignedBytesEqual((signed char)(expected), (signed char)(actual), text, file, line); } while(0)
...
#define DOUBLES_EQUAL_LOCATION(expected, actual, threshold, text, file, line)\
  do { UtestShell::getCurrent()->assertDoublesEqual((double)(expected), (double)(actual), (double)(threshold), text, file, line); } while(0)
